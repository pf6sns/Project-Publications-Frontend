...
export function TempAdminDashboard({
  ...
  const [reviewedFileName, setReviewedFileName] = useState(null);
  ...
  const assignedPubs = publications.filter(p => {
    if (currentUser.tempAdminAccessType === 'full') {
      return p.authorId !== currentUser.id && (p.status === 'Pending' || p.assignedReviewerId === currentUser.id);
    }
    return p.assignedReviewerId === currentUser.id;
  });

  // Compute metrics specifically for assigned publications
  const pendingCount = assignedPubs.filter(p => p.status === 'Pending').length;
  const approvedCount = assignedPubs.filter(p => p.status === 'Approved').length;
  const totalCount = assignedPubs.length;

  const activePub = publications.find(p => p.id === selectedPubIdForReview);
  const authorUser = activePub ? mockUsers[activePub.authorId] : undefined;
  ...
  const handleApproveAction = () => {
    ...
    const finalFeedback = 'Approved. Annotated document attached.';
    onApprove(activePub.id, finalFeedback, reviewedFileName);
    setReviewedFileName(null);
    onSelectReview(null);
  };

  if (activePub) {
    // Determine number of remaining attempts for scholars
    const maxVersions = 3;
    const remainingAttempts = Math.max(0, maxVersions - activePub.currentVersion);
      ...
      <div className="space-y-6 animate-fade-in text-left">
        ...
        <div className="flex flex-col gap-6 text-left">
          ...
          <div className="bg-pure-white p-6 rounded-2xl border border-platinum-silver shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs leading-normal">
                  <div className="border border-platinum-silver/60 p-3 rounded-lg bg-pure-white shadow-3xs flex items-center justify-between col-span-2 flex-wrap gap-2">
                    <div>
                      ...
                      <span className="font-mono font-bold text-charcoal truncate block max-w-50" title={activePub.versions[activePub.versions.length - 1].fileName}>
                        {activePub.versions[activePub.versions.length - 1].fileName}
                      </span>
                    </div>
                    <button
                      onClick={() => downloadFile(activePub.id, activePub.currentVersion, activePub.versions[activePub.versions.length - 1].fileName)}
                  ...
                  <div className="border border-platinum-silver/60 p-3 rounded-lg bg-pure-white shadow-3xs col-span-2">
                    <span className="text-slate-gray block font-semibold text-[9px] uppercase">Current Version</span>
                    <span className="font-bold text-charcoal">Version {activePub.currentVersion}</span>
                  </div>
                </div>

                {/* Upload Dates Timeline */}
                <div className="border-t border-platinum-silver/60 pt-3">
                  <span className="text-slate-gray block font-semibold text-[9px] uppercase mb-2">Submission Timeline</span>
                  {activePub.versions.length > 1 ? (
                    <div className="flex flex-col sm:flex-row gap-3">
                      <div className="flex-1 bg-pure-white p-3 rounded-lg border border-platinum-silver/45 flex flex-col justify-center text-left">
                        <span className="text-[9px] uppercase tracking-wider text-slate-gray font-semibold block mb-0.5">Original Submission Date (V1)</span>
                        <span className="font-bold text-charcoal text-xs">{new Date(activePub.versions[0].uploadDate).toLocaleString()}</span>
                      </div>
                      <div className="flex-1 bg-pure-white p-3 rounded-lg border border-platinum-silver/45 flex flex-col justify-center text-left">
                        <span className="text-[9px] uppercase tracking-wider text-slate-gray font-semibold block mb-0.5">Latest Reuploaded Date (V{activePub.currentVersion})</span>
                        <span className="font-bold text-charcoal text-xs">{new Date(activePub.versions[activePub.versions.length - 1].uploadDate).toLocaleString()}</span>
                      </div>
                    </div>
                  ) : (
            ...
            {/* Version timeline withAttempts remaining */}
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <h3 className="text-xs font-semibold uppercase text-slate-gray tracking-wider font-sans">
                  Version Timeline
                </h3>
                <span className="text-[10px] text-charcoal bg-frost-gray font-bold px-2 py-0.5 rounded-full border border-platinum-silver">
                  {remainingAttempts} Attempts Remaining
                </span>
              </div>

              {/* Graphical Timeline visualization */}
              <div className="border border-platinum-silver rounded-xl p-4 bg-ice-gray/50 space-y-3">
                <div className="flex flex-col space-y-3">
                  {activePub.versions.map((v, i) => (
                    <div key={i} className="flex items-start space-x-3 text-xs text-left">
                      <div className="flex flex-col items-center mt-1">
                        <div className={`h-5 w-5 rounded-full font-bold text-[10px] flex items-center justify-center border ${
                          v.status === 'Approved' ? 'bg-frost-gray text-charcoal border-platinum-silver' :
                          'bg-ice-gray text-charcoal border-platinum-silver'
                        }`}>
                          {v.version}
                        </div>
                        {i < activePub.versions.length - 1 && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="font-semibold text-charcoal">
                            {v.version === 1 ? 'Version 1: Initial Submission' :
                             v.version === 2 ? 'Version 2: First Resubmission' :
                             'Version 3: Final Resubmission'}
                          </span>
                          <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-mono ${
                            v.status === 'Approved' ? 'bg-frost-gray text-charcoal border border-platinum-silver' :
                            'bg-ice-gray text-charcoal border-platinum-silver'
                          }`}>
                            {v.status}
                          </span>
                        </div>
                        <p className="text-[10px] text-slate-gray mt-0.5 font-mono">
                          Uploaded: {v.fileName} ({v.fileSize}) • {new Date(v.uploadDate).toLocaleDateString()}
                        </p>
                        {v.feedback && (
                          <p className="text-[10px] text-slate-gray bg-pure-white p-2 rounded border border-platinum-silver mt-1 italic">
                            "{v.feedback}"
                          </p>
                        )}
                      </div>
                  ...
                  <div className="border border-slate-200 bg-emerald-50/20 rounded-xl p-3 flex items-center justify-between shadow-xs animate-fade-in">
                    <div className="flex items-center space-x-2.5 truncate">
                      ...
                      <div className="truncate text-left">
                        <p className="text-xs font-bold text-slate-800 truncate">{reviewedFileName}</p>
                        <p className="text-[10px] text-emerald-700 font-semibold font-mono">Annotated file attached</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setReviewedFileName(null)}
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className="bg-pure-white p-4 rounded-2xl border border-platinum-silver silver-glow flex flex-col justify-between text-left shadow-xs transition-all duration-300 hover:scale-105 hover:shadow-md hover:border-platinum-silver/80">
          <span className="text-[10px] uppercase font-semibold text-charcoal">Approved</span>
          <span className="text-2xl font-bold font-mono text-charcoal mt-1">{approvedCount}</span>
        </div>
      </div>
                ...
                <div key={pub.id} className="pt-4 first:pt-0 space-y-2 text-left">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-black font-mono border ${
                      pub.status === 'Approved' ? 'bg-frost-gray text-charcoal border-platinum-silver' :
                      'bg-ice-gray text-charcoal border-platinum-silver'
                    }`}>
                      {pub.status}
                    </span>
                  ...
                  <div className="flex justify-between items-center text-[10px] text-slate-gray border-t border-slate-100 pt-2 font-medium">
                    <div>
                      <p>Date: {new Date(pub.submissionDate).toLocaleDateString()}</p>
                  ...
                  {assignedPubs.map((pub) => (
                    <tr key={pub.id} className="hover:bg-ice-gray/30 transition-colors">
                      ...
                      <td className="p-4 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-black font-mono border ${
                          pub.status === 'Approved' ? 'bg-frost-gray text-charcoal border-platinum-silver' :
                          pub.status === 'Pending' ? 'bg-ice-gray text-charcoal border-platinum-silver' :
