...
export function AdminReviews({
  ...
  const activeAuthor = activePub ? users[activePub.authorId] : undefined;
        ...
        <div className="bg-pure-white rounded-xl border border-platinum-silver shadow-sm transition-all duration-300 hover:scale-[1.005] hover:shadow-md overflow-hidden flex flex-col">
          ...
          <div className="p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-50/30">
            ...
            <div className="flex-2 text-center w-full md:w-auto">
              ...
              <p className="text-sm text-slate-gray font-medium">Author: {activeAuthor.name}</p>
            </div>

            <div className="flex-1 text-right w-full md:w-auto flex justify-end">
              <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border inline-block ${
                activePub.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 
                activePub.status === 'Rejected' ? 'bg-red-50 text-red-600 border-red-200' :
          ...
          <div className="p-6 md:p-8 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {(() => {
                const latestVersion = activePub.versions[activePub.versions.length - 1];
                return (
                  <div className="border border-platinum-silver/60 p-4 rounded-xl bg-slate-50/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-left">
                        <h4 className="font-bold text-charcoal text-sm truncate max-w-sm" title={latestVersion.fileName}>{latestVersion.fileName}</h4>
                        <p className="text-xs text-slate-gray font-mono mt-1">Uploaded {new Date(latestVersion.uploadDate).toLocaleString()}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => downloadFile(activePub.id, activePub.versions.length, latestVersion.fileName)}
                      className="px-5 py-2.5 bg-frost-gray hover:bg-mist-silver border border-platinum-silver rounded-lg text-charcoal flex items-center justify-center space-x-2 cursor-pointer transition-colors text-xs font-bold w-full sm:w-auto shrink-0"
          ...
          {/* 3. Bottom Section: Upload Reviewed Document */}
                ...
                <div className="border border-emerald-200 bg-emerald-50/30 rounded-xl p-5 flex items-center justify-between shadow-xs animate-fade-in">
                  <div className="flex items-center space-x-4 truncate">
                    ...
                    <div className="truncate text-left">
                      <p className="text-sm font-bold text-slate-800 truncate">{reviewedFileName}</p>
            ...
            >
              <div className="space-y-1 text-left">
                ...
                <p className="text-xs text-slate-gray font-medium">
                  Author: {pub.author} • Division: {pub.department}
                </p>
              </div>
...
export function AdminAnalyticsView({ publications }) {
  // Aggregate statistics per faculty member
  const facultySummaries = {};

  publications.forEach(pub => {
    if (!facultySummaries[pub.authorId]) {
      const user = mockUsers[pub.authorId];
      facultySummaries[pub.authorId] = {
        authorId: pub.authorId,
        name: pub.author || (user ? user.name : 'Unknown Author'),
        department: pub.department || (user ? user.department : 'Unknown Dept'),
        total: 0,
        approved: 0,
        rejected: 0,
        rate: 0,
    ...
    const s = facultySummaries[pub.authorId];
    s.total += 1;
    if (pub.status === 'Approved') s.approved += 1;
    if (pub.status === 'Rejected') s.rejected += 1;
  });

  // Calculate rate
  const summariesList = Object.values(facultySummaries).map(s => {
    const nonPending = s.total - (publications.filter(p => p.authorId === s.authorId && p.status === 'Pending').length);
    s.rate = nonPending > 0 ? Math.round((s.approved / nonPending) * 100) : 100;
    return s;
  });
    ...
    <div className="bg-pure-white rounded-xl border border-platinum-silver shadow-xs overflow-hidden animate-fade-in transition-all duration-300 hover:scale-[1.005] hover:shadow-md">
      <div className="p-4 border-b border-platinum-silver bg-ice-gray flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-gray">Aggregated submission statistics and acceptance rates</p>
        </div>
      </div>
        ...
        {summariesList.map((f, idx) => (
          <div key={idx} className="p-4 space-y-2 text-xs text-left">
            ...
            <div className="grid grid-cols-3 gap-2 text-center pt-2 text-[10px]">
              ...
              <div className="bg-slate-50 p-1.5 rounded border border-slate-100">
                <span className="block text-[8px] uppercase text-slate-400 font-bold">Approved</span>
                <span className="font-bold font-mono text-charcoal text-xs">{f.approved}</span>
              </div>
              <div className="bg-slate-50 p-1.5 rounded border border-slate-100">
                <span className="block text-[8px] uppercase text-slate-400 font-bold">Rejected</span>
                <span className="font-bold font-mono text-red-655 text-xs">{f.rejected}</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-ice-gray text-slate-gray uppercase tracking-widest font-extrabold text-[9px] border-b border-platinum-silver">
              ...
              <th className="p-4 text-center text-charcoal font-bold">Approved</th>
              <th className="p-4 text-center text-red-650 font-bold">Rejected</th>
              <th className="p-4 text-right">Acceptance Rate</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-platinum-silver text-slate-gray">
            {summariesList.map((f, idx) => (
              <tr key={idx} className="hover:bg-ice-gray/30 transition-colors font-medium">
                ...
                <td className="p-4 text-center text-charcoal font-mono font-bold">{f.approved}</td>
                <td className="p-4 text-center text-red-650 font-mono font-bold">{f.rejected}</td>
