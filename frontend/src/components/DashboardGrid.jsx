...
export function FacultyDashboardCharts({ publications, publicationStatuses }) {
  ...
  const donutData = [
    { name: 'Approved', value: publicationStatuses.approved, color: statusColors.Approved },
                  ...
                  <PieChart>
                    ...
                    >
                      {donutData.map((entry, index) => {
                        const fillValue = entry.name === 'Approved'
                          ? 'url(#approvedPieGrad)'
                          : 'url(#pendingPieGrad)';
        ...
        <div className="chart-sm w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={departmentSubmissions} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="approved" stackId="a" fill="#16A34A" name="Approved" barSize={20} />
